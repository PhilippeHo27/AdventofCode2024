  ...
  void PrintSolution(const std::filesystem::path& inputFile, bool shouldRender)
	{
		...
		for (const auto& [symbol, positions] : terrains)
		{
			...
			for (const auto& region : regions)
			{
				...
				// Count active edges for this region
				int activeEdgeCount = 0;
				for (const auto& quad : quadsForRegion)
				{
					for (const auto& edge : quad.edges)
					{
						if (edge.active)
						{
							activeEdgeCount++;
				...
				std::cout << "Region size: " << region.size() << " quads with " << activeEdgeCount << " active edges" << std::endl;
				std::cout << "Region value: " << region.size() * activeEdgeCount << std::endl;

				totalSum += (region.size() * activeEdgeCount);
			}
		}

		std::cout << "\nFinal sum: " << totalSum << std::endl;
	}
} // namespace Puzzle12A